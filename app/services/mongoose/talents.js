...
const getAllTalents = async (req) => {
  ...
  if (keyword) {
    // regex dan option ini untuk filter jika mau filter, ketika kita cari, kemudian di ketik misalkan r maka di akan merekomendasikan data yang ada r nya, i untuk string
    condition = { ...condition, name: { $regex: keyword, $options: "i" } };
  }

  const result = await Talents.find(condition)
    .populate({
      // jika kita ingin mengambil semua data image maka gunakan seperti ini : .populate('image);
      path: "image",
      select: "_id name", // jika kita tidak menggugunaka popolute maka yang akan didapatka hanya idnya saja tidak imagenya
    })
